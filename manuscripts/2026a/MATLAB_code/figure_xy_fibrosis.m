function figure_xy(options)
    ...
    arguments
        ...
        options.marker_fibrosis = 's'
        options.marker_size_fibrosis = 25
        options.marker_face_color_fibrosis = [0.5 1 1]
        options.marker_face_alpha_fibrosis = 1
        options.marker_edge_color_fibrosis = [0 0 0]
        ...
        options.legend_position_2 = [1.05 0.87]
        options.legend_vertical_scale_2 = 1.4
        ...
        options.output_image_file = "../figures/fig_xy_fibrosis"
    ...
    c.Position = c.Position + [0.21 0.04 0 -0.52];
    c.Label.String = "    dPdt" + newline + "(mmHg s^{-1})";
    c.Label.HorizontalAlignment = "center";
    c.Label.Rotation = 0;
    c.Label.FontSize = options.legend_font_size;;
    c.Label.Position([1 2]) = c.Label.Position([1 2]) + [0.5 950];

    % Add in fibrosis
    vi = find( strcmp(d.class_1, "material") & ...
                (d.total_change_1 > 0) & ...
                (d.time > options.scan_parameters_min_time) & ...
    ...
    h_1(2) = scatter(x, y, ...
                options.marker_size_fibrosis, ...
                options.marker_fibrosis, ...
                MarkerFaceColor = options.marker_face_color_fibrosis, ...
                MarkerFaceAlpha = options.marker_face_alpha_fibrosis, ...
                MarkerEdgeColor = options.marker_edge_color_fibrosis);

    p = polyfit(x, y, 1);
    x_fit = linspace(min(x), max(x), 100);
    y_fit = polyval(p, x_fit);
    h_temp = plot(x_fit, y_fit, '-', ...
                Color = 0 * options.marker_face_color_fibrosis);
