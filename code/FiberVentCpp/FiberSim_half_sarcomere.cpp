...
FiberSim_half_sarcomere::FiberSim_half_sarcomere(FiberSim_muscle* set_p_parent_fs_muscle, int set_hs_id)
{
    ...
    // Calculate the x positions
    calculate_x_max_iterations = calculate_x_positions();
...
void FiberSim_half_sarcomere::calculate_sparse_k_bare(void)
{
...
void FiberSim_half_sarcomere::calculate_sp_F_and_G(gsl_vector* x)
{
...
size_t FiberSim_half_sarcomere::calculate_x_positions_sparse()
{
...
size_t FiberSim_half_sarcomere::calculate_x_positions_ye_method(void)
{
...
double FiberSim_half_sarcomere::calculate_force(double delta_hsl, double time_step)
{
    ...
    holder = (holder * (1.0 - p_parent_fs_muscle->p_parent_muscle->muscle_prop_fibrosis) *
...
double FiberSim_half_sarcomere::calculate_titin_force(void)
{
    ...
    for (int m_counter = 0; m_counter < m_n; m_counter++)
    {
        ...
        for (int a_counter = 0; a_counter < 6; a_counter++)
        {
            ...
            // Calculate as sum of linear and exponential forces
            holder = holder + 
                (t_k_stiff * (x_m - x_a - t_offset)) +
                (GSL_SIGN(x_m - x_a) * t_sigma *
                    (exp(fabs(x_m - x_a - t_offset) / t_L) - 1));
        }
    }
...
double FiberSim_half_sarcomere::calculate_extracellular_force(void)
{
    ...
    pas_force = p_parent_fs_muscle->p_parent_muscle->muscle_prop_fibrosis *
                    ((e_k_stiff * (hs_length - e_slack_length)) +
                        (GSL_SIGN(hs_length - e_slack_length) * e_sigma *
                        (exp(fabs(hs_length - e_slack_length) / e_L) - 1.0)));
...
void FiberSim_half_sarcomere::write_hs_status_to_file(char output_file_string[])
{
    ...
    fprintf_s(output_file, "\t\"prop_fibrosis\": %.*g,\n", 
        p_fs_options->dump_precision, p_parent_fs_muscle->p_parent_muscle->muscle_prop_fibrosis);
